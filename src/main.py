...
from train import adapter_train
from utils import (
set_seed, 
set_logger, 
read_entities, 
...
if __name__ == '__main__':
    ...
    for data_type in tasks:
        ...
        args.log()
        data = EMDataset(data_type)
        # entities
        data.left_entities, data.right_entities = read_entities(data_type, args)
        # samples
        data.train_pairs, data.train_y, \
        data.train_un_pairs, data.train_un_y = read_ground_truth_few_shot(f"data/{data_type}", ["train"], k=args.k, return_un_y=True)
