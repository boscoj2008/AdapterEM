"""
 ...
 - codes adapted from PromptEM paper https://arxiv.org/abs/2207.04802
"""
...
from mlm_data import GEMData
from args import MLMArgs
import os
import csv
...
def read_rel_entities(file_path: str, args, add_token=True, summarize=False):
    entities = []
    file_path += ".csv"
    ...
    if summarize:
        summarizer = Summarizer(entities, args.model_name_or_path)
        new_entities = []
...
def read_semi_entities(file_path: str, args, skip=True, add_token=True, summarize=False):
    entities = []
    file_path += ".json"
    with open(file_path, "r") as rd:
        lines = json.load(rd)
    ...
    if summarize:
        summarizer = Summarizer(entities, args.model_path_or_name)
        new_entities = []
        for ent in tqdm(entities, desc="summarizing..."):
            new_entities.append(summarizer.transform_sentence(ent))
        entities = new_entities
...
def read_text_entities(file_path: str, args, add_token=False, summarize=False):
    entities = []
    file_path += ".txt"
    with open(file_path, "r") as rd:
        lines = rd.readlines()
...
def read_entities(data_type: str, args: MLMArgs):
    read_entities_fun = read_entities_funs[data_type]
    left_entities = read_entities_fun[0](f"data/{data_type}/left", args, add_token=args.add_token,
                                         summarize=args.text_summarize)
    right_entities = read_entities_fun[1](f"data/{data_type}/right", args , add_token=args.add_token,
                                          summarize=args.text_summarize)
...
def statistic_of_current_train_set(data: GEMData):
    acc = 0
    neg = 0
    pos = 0
